...
async function recalcStatusPagar(contaId, emp, uid) {
  const r = await query(`
    ...
    OUTER APPLY (
      ...
      WHERE conta_id = cp.id AND estornado = 0
    ) ag
  ...
  await query(`
    ...
    SET status = @st, valor_pago = @vp,
        data_pagamento = CASE WHEN @st = 'pago' THEN GETDATE() ELSE data_pagamento END,
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (busca)     { baseWhere += ` AND (cp.fornecedor LIKE @bsc OR ISNULL(cp.descricao,'') LIKE @bsc OR ISNULL(cp.numero_documento,'') LIKE @bsc)`; params.bsc = `%${busca}%`; }
    ...
    let where = baseWhere;
    if (status === 'vencido') {
      where += ` AND cp.status IN ('pendente','parcial') AND cp.data_vencimento < CAST(GETDATE() AS DATE)`;
    } else if (status) {
    ...
    const r = await query(`
      SELECT cp.id, cp.tipo, cp.fornecedor, cp.categoria, cp.descricao, cp.numero_documento,
             ...
             CASE WHEN cp.status IN ('pendente','parcial') AND cp.data_vencimento < CAST(GETDATE() AS DATE)
                  THEN DATEDIFF(DAY, cp.data_vencimento, GETDATE()) ELSE 0 END AS dias_atraso
      FROM ContasPagar cp
      OUTER APPLY (
        SELECT SUM(valor_pago) AS total_pago
        FROM PagamentosContasPagar WHERE conta_id = cp.id AND estornado = 0
      ) ag
      WHERE ${where}
      ORDER BY
    ...
    const tot = await query(`
      SELECT
        ...
        COALESCE(SUM(CASE WHEN cp.status IN ('pendente','parcial') AND cp.data_vencimento < CAST(GETDATE() AS DATE)
        ...
        SUM(CASE WHEN cp.status IN ('pendente','parcial') AND cp.data_vencimento < CAST(GETDATE() AS DATE) THEN 1 ELSE 0 END) AS qtd_vencido
      FROM ContasPagar cp
      OUTER APPLY (
        SELECT SUM(valor_pago) AS total_pago
        FROM PagamentosContasPagar WHERE conta_id = cp.id AND estornado = 0
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const cab = await query(`
      SELECT cp.*,
             (cp.valor - COALESCE((SELECT SUM(valor_pago) FROM PagamentosContasPagar WHERE conta_id = cp.id AND estornado = 0), 0)) AS valor_aberto
      FROM ContasPagar cp
...
router.post('/:id/pagar', auth, async (req, res) => {
  try {
    ...
    const rec = await query(`
      SELECT cp.valor, cp.status,
             COALESCE((SELECT SUM(valor_pago) FROM PagamentosContasPagar WHERE conta_id = cp.id AND estornado = 0), 0) AS total_pago
...
router.put('/:id/cancelar', auth, async (req, res) => {
  try {
    ...
    const emp = req.user.empresa_id;
    await query(
      `UPDATE ContasPagar SET status = 'cancelado', atualizado_em = GETDATE()
